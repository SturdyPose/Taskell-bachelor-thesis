...
konkurencí, či paralelizací problému. Toto DSL je identifikováno jako \textbf{Haskallyzer}.
Pro řešení této problematiky byl zvolen Haskell, jelikož se zdá jako nejoptimálnější. Jsou zde rozsáhlé knihovny pro parsování jazyka (Parsec, MegaParse, AttoParsec),
...
let exampleAnnotatedProcess = exampleConcurrentProcess => "kalmanFilter" 
                                                          -> writeToIO
                                                       => "gaussianFilter" 
                                                          -> writeToIO
...
\subsection{Vysvětlivka}

Načte se csv soubor do proměné exampleCSV. Při kompilaci se kontrolují v csv souboru dvojice "(a, Int)" a to název sloupce i o jaký datový typ se jedná. 
Atribut CompileTime umožňí uložit hodnoty během kompilace a tím není zapotřebí načíst 
soubor při běhu programu. Nevýhodou je, že se tím zaplní paměť při spuštění programu, ale vzato není zapotřebí IO monáda a hodnoty
jsou ihned připraveny.
